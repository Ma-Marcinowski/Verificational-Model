...
def CoreNet(ix, path):
    ...
    x = Conv2D(16, (12, 12), strides=1, dilation_rate=4, padding='same', activation='relu', name='1stConv' + path)(ix)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)

    x = Conv2D(32, (6, 6), strides=1, dilation_rate=2, padding='same', activation='relu', name='2ndConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)

    x = Conv2D(64, (3, 3), strides=1, dilation_rate=1, padding='same', activation='relu', name='3rdConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
...
model.summary(line_length=None,
              positions=None,
              print_fn=None)
tf.keras.utils.plot_model(model, to_file='/plot/save/path/VM_v2.6.2.png', show_shapes=True, show_layer_names=True, rankdir='TB')
