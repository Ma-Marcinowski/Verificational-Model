...
def CoreNet(ix, path):
    ...
    x = Conv2D(16, (12, 12), strides=4, padding='same', activation='relu', name='1stConv' + path)(ix)
    ...
    x = Conv2D(32, (6, 6), strides=2, padding='same', activation='relu', name='2ndConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
...
model.summary(line_length=None,
              positions=None,
              print_fn=None)
tf.keras.utils.plot_model(model, to_file='/path/VM_v2.6.3.png', show_shapes=True, show_layer_names=True, rankdir='TB')
