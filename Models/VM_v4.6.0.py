...
def CoreNet(ix, path):
    ...
    x = Conv2D(16, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='1stConv' + path)(ix)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
    x = Conv2D(16, (3, 3), strides=1,dilation_rate=2, padding='same', activation='relu', name='2ndtConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
    x = Conv2D(16, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='3rdConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)

    x = Conv2D(32, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='4thConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
    x = Conv2D(32, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='5thConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
    x = Conv2D(32, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='6thConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)

    x = Conv2D(64, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='7thConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
    x = Conv2D(64, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='8thConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
    x = Conv2D(64, (3, 3), strides=1, dilation_rate=2, padding='same', activation='relu', name='9thConv' + path)(x)
    x = BatchNormalization(axis=-1, scale=True, trainable=True)(x)
...
model.summary(line_length=None,
              positions=None,
              print_fn=None)
tf.keras.utils.plot_model(model, to_file='/content/VM_v4.6.0.png', show_shapes=True, show_layer_names=True, rankdir='TB')
