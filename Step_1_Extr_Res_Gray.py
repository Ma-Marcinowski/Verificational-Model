...
#Funkcja {glob.glob('dowolna_nazwa.format_pliku')} dokonuje identyfikacji wszystkich plików o danym formacie 
#w folderze gdzie wykonywany jest program.
tifs = glob.glob('*.tif')
...
for j in tqdm(tifs):
    ...
    #Wczytaj j w skali szarości, innymi słowy {cv2.imread(wczytywany obraz, zakres barw)}.
    img = cv2.imread(j, 0)
    ...
    #Funkcja {wskazany_obraz [y:y+h, x:x+w]} wycina fragment obrazu określony współrzędnymi (od y do h, od x do w).
    ...
    #pod swoją lub zmodyfikowaną nazwą, w wybranym formacie.
