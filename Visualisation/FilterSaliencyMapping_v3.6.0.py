...
def FeatureGradients(image, feature_index, feature_layer):
    ...
    with tf.GradientTape() as tape:
        ...
        given_feature_value = feature_layer_output[:, feature_index] #Globally average pooled activation value

    feature_gradients = tape.gradient(given_feature_value, image)

    positive_gradients = tf.nn.relu(feature_gradients)

    small_gradients = tf.math.divide(1, tf.math.add(positive_gradients, 1e-9))

    return small_gradients, given_feature_value

def FilterBackpropSaliencyMapping(model_load_path, img_out_path, images_dir, input_layer_name, output_layer_name, num_of_features):
    ...
    for img_path, img_name in tqdm(images, total=len(image_paths), desc=output_layer_name + ' features mapping:', leave=True):
        ...
        for feature_index in tqdm(range(num_of_features), desc='Image features mapping:', leave=False):
            ...
            feature_gradients, feature_value = FeatureGradients(image=initial_image,
                                                                feature_index=feature_index,
                                                                feature_layer=feature_layer)

            grads_tensor = feature_gradients.numpy()
            grads_matrix = grads_tensor[0, :, :, 0]

            value_scalar = feature_value.numpy()

            thv, filter_img = cv2.threshold(raw_image, 55, 255, cv2.THRESH_TOZERO)
            filtered_matrix = np.multiply(grads_matrix, filter_img)

            bgr_sal_map = cv2.merge((raw_image, filtered_matrix, empty_img))

            ind = str(feature_index + 1).zfill(nulls)

            cv2.imwrite(img_out_path + output_layer_name + '_image_[' + img_name + ']_feature_maps_' + str(value_scalar) + '_' + ind + '.png', bgr_sal_map)
